...
def homePage(Request):
    ...
    return render( Request, "index.html",{'product':products,'testimonials':testimonials})
...
def searchPage(Request):
    if(Request.method=="POST"):
        ...
        subcategory=Subcategory.objects.all().order_by("-id")
        brand=Brand.objects.all().order_by("-id")
        return render(Request,"shop.html",{'product':products,'maincategory':maincategory,'subcategory':subcategory,'brand':brand,'mc':"All",'sc':"All",'br':"All"})
    else:
...
def confirmationPage(Request,id):
    try:
        ...
        if(subtotal>0 and subtotal<1000):
            shipping=150
        total=subtotal+shipping
        return render(Request,"confirmation.html",{'buyer':buyer,'cart':cart,'subtotal':subtotal,'shipping':shipping,'total':total,'checkout':checkout})
    except:
        return HttpResponseRedirect("/admin/")
...
def newslatterSubscribePage(Request):
    if(Request.method=="POST"):
        ...
        try:
            ...
            error(Request,"Your Email Id is Already Subscribe!!! ")
        return HttpResponseRedirect("/")
    else:
        return HttpResponseRedirect("/")
...
def forgetPassword2Page(Request):
    ...
    if(Request.method=="POST"):
        if(username):
            ...
            error(Request,"UnAuthorized Activity")
    if(username):
